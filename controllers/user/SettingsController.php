    ...
    public function actionProfile()
    {
        ...
        if (\Yii::$app->request->post()) {
            ...
            if ($model->validate()) {
                ...
                $old_picture = $model->getAttribute('picture');
                if (!empty($old_picture)) {
//                    getcwd() - get current working directory
                    $file_to_remove = getcwd().'/img/uploads/' . $old_picture;
                    ...
                    for ($i = 1, $filename = $model->picture->name; file_exists(getcwd() . '/img/uploads/' . $filename); $i++){
                        $filename =  $model->picture->getBaseName() . $i . '.' .$model->picture->getExtension();
                    }
