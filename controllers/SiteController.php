...
class SiteController extends Controller
{
    ...
    public function actionUsers($id) {
        ...
        $query->select('court.id as id, address,type_id,lat,lon')
            ->from('court_bookmark, court')
            ->where(['user_id' => $id])
            ->andWhere('court_id = court.id');
        $courts = $query->all();
        ...
        foreach ($courts as $court) {
            ...
            {
                $photo[$i] = $query->one();
                $courts[$i]['photo'] = $photo[$i]; 
            }
            else
            {
                $photo[$i]['photo'] = 'defaultCourt.jpg';
                $courts[$i]['photo'] = 'defaultCourt.jpg'; 
            }

            $queryCount = new Query();
            $queryCount->select('user_id')
                ->from('court_bookmark')
                ->where(['court_id' => $court['id']]);
            $countPl = $queryCount->all();
            $courts[$i]['count'] = count($countPl);
        ...
        $users = [];
            ...
            $query_games->select('game.id as id,court.id as court_id, address, time, sport_type.name as sport, need_ball,sport_type_id')
                ->from('court,game,game_user,sport_type')
                ->where(['user_id' => $id])
                ->andWhere('game_id = game.id')
                ->andWhere('court_id = court.id')
                ->andWhere(['>=', 'time',date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s')))])
